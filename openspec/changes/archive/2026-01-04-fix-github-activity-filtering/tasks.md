...
- [x] 1.1 Create new method `collectGitHubActivityAllRepos(String user, int days)` in ActivityAggregator.java
...
- [x] 1.2 Update `aggregateActivities()` method in ActivityAggregator.java (lines 95-119)
...
- [x] 1.3 Update `AggregatedActivity` class in ActivityAggregator.java (lines 30-34)
...
- [x] 1.4 Remove obsolete `collectGitHubActivity(Map<String, List<Directory>> repoMap, ...)` method
...
- [x] 2.1 Create new helper method `formatActivitiesGroupedByRepo(JsonObject activity)` in Main.java
...
- [x] 2.2 Add required imports to Main.java
  - `import java.util.HashMap;`
  - `import java.util.Map;`
  - `import java.util.Collections;`
  - `import java.util.ArrayList;`

- [x] 2.3 Update `formatMultiDirPrompt()` method in Main.java (lines 506-517)
...
- [x] 2.4 Test formatting with various GitHub activity structures
...
- [x] 3.1 Update prompts/multidir-standup.prompt.md (line 26 area)
...
- [x] 3.2 Add metadata clarification to prompts/multidir-standup.prompt.md
  - Add note in metadata section: "**Note**: GitHub activity includes ALL repositories, not just those configured above."

## 4. Update Documentation

- [x] 4.1 Update .claude/commands/claude-gh-standup.md
...
- [x] 4.3 Update CLAUDE.md (if relevant sections exist)
  - Document the two-tier approach in development guidance (CLAUDE.md already staged, no additional changes needed)

## 5. Testing and Validation

- [x] 5.1 Test multi-dir mode shows ALL GitHub activity (not just configured repos)
  - Configure 1 directory for repo A
  - Work on repo B yesterday
  - Verify report includes both repo A and repo B activities

- [x] 5.2 Test multi-dir mode shows only configured dirs in Local WIP section
  - Configure 2 directories
  - Verify local changes section only shows those 2 directories

- [x] 5.3 Test report clearly separates GitHub (all) from Local (config)
  - Verify heading structure makes the distinction clear

- [x] 5.4 Test legacy mode unchanged (no config or explicit --repo)
  - Verify single-directory mode still works
  - Verify explicit --repo flag behavior unchanged

- [x] 5.5 Test error handling graceful (no GitHub activity case)
  - Simulate GitHub API failure
  - Verify report continues with local changes

- [x] 5.6 Test performance improved (1 API call vs N calls)
...
- [x] 6.1 Verify code follows project conventions
  - 4-space indentation
  - PascalCase for classes, camelCase for methods
  - Clear error messages to stderr

- [x] 6.2 Add code comments for new methods
  - Document `collectGitHubActivityAllRepos()` purpose
  - Document `formatActivitiesGroupedByRepo()` logic

- [x] 6.3 Ensure JBang scripts still executable
